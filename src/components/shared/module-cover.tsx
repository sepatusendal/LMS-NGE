...
export function ModuleBadge({ module }: { module: ClassModule }) {
  ...
  const t = useTranslations("moduleCover");
  return (
    <a
    ...
    >
      ...
      {t("moduleName", { name: module.curriculumName })}
    </a>
  );
}
  ...
  if (!module) {
    return (
      <div className="border-border/60 bg-muted/30 flex items-center gap-3 rounded-2xl border border-dashed px-4 py-3.5">
        ...
        <p className="text-muted-foreground text-sm">{t("noModule")}</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="relative min-w-0 flex-1">
        <p className="text-xs font-medium text-white/80">{t("learningModule")}</p>
