...
// The delete-slots / delete-overrides / upsert-slots steps run atomically
// inside sync_class_schedule_slots() (20260915060000) — three separate
// client calls meant an upsert failure after the deletes already succeeded
// could silently leave a class missing a schedule slot for a day it was
// supposed to still meet on. See that migration for the full writeup.
async function syncScheduleSlots(supabase: ReturnType<typeof createClient>, classId: string, input: ClassInput) {
  ...
  const { error } = await supabase.rpc("sync_class_schedule_slots", {
    p_class_id: classId,
    p_days: days,
    p_slots: toSlots(classId, input),
  });
  if (error) throw error;
}
